        ...
        for root_name, platform_paths in result.items():
            for platform in platform_paths:
                ...
                if new_roots[root_name][shotgun_path_key] is None:
                    ...
                    expected_path = "%s\\%s" % (
                        new_roots[root_name][shotgun_path_key],
                        project_name,
                    )
                else:
                    expected_path = "%s/%s" % (
...
class TestGetPrimaryRoot(TankTestBase):
    ...
    def test_non_project_path(self):
        ...
        """
        non_project_path = os.path.join(
            os.path.dirname(self.project_root), "xxxyyyzzzz"
        )
        self.assertRaises(
            TankError, tank.pipelineconfig_factory.from_path, non_project_path
        )
