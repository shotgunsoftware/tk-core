...
class TestValidate(TestTemplatePath):
    ...
    def test_missing_directories(self):
        relative_path = os.path.join(
            "shots", "seq_1", "Anm", "work", "shot_1.mmm.v001.002.ma"
        )
    ...
    def test_missing_version(self):
        relative_path = os.path.join(
            "shots", "seq_1", "shot_1", "Anm", "work", "shot_1.mmm.002.ma"
        )
        invalid_path = os.path.join(self.project_root, relative_path)
        self.assertFalse(self.template_path.validate(invalid_path))
    ...
    def test_skipping_keys_key_present(self):
        ...
        result = self.template_path.validate(
            self.valid_path, fields=input_fields, skip_keys=["snapshot"]
        )
        self.assertTrue(result)
    ...
    def test_skip_key_type_not_match(self):
        relative_path = os.path.join(
            "shots", "seq_1", "shot_1", "Anm", "work", "shot_1.mmm.v001.###.ma"
        )
        ...
        result = self.template_path.validate(
            valid_path, fields=input_fields, skip_keys=["snapshot"]
        )
        self.assertTrue(result)
    ...
    def test_optional_values(self):
        ...
        test_path = os.path.join(
            self.project_root, "shots", "seq_1", "s1", "Anm", "work", "s1.mmm.v003"
        )
        self.assertTrue(template.validate(test_path))

        # branch T   version F   snapshot T
        test_path = os.path.join(
            self.project_root, "shots", "seq_1", "s1", "Anm", "work", "s1.mmm.002.ma"
        )
        self.assertTrue(template.validate(test_path))

        # branch T   version F   snapshot F
        test_path = os.path.join(
            self.project_root, "shots", "seq_1", "s1", "Anm", "work", "s1.mmm"
        )
        self.assertTrue(template.validate(test_path))

        # branch F   version T   snapshot T
        test_path = os.path.join(
            self.project_root, "shots", "seq_1", "s1", "Anm", "work", "s1.v003.002.ma"
        )
        self.assertTrue(template.validate(test_path))

        # branch F   version T   snapshot F
        test_path = os.path.join(
            self.project_root, "shots", "seq_1", "s1", "Anm", "work", "s1.v003"
        )
        self.assertTrue(template.validate(test_path))

        # branch F   version F   snapshot T
        test_path = os.path.join(
            self.project_root, "shots", "seq_1", "s1", "Anm", "work", "s1.002.ma"
        )
        self.assertTrue(template.validate(test_path))

        # branch F   version F   snapshot F
        test_path = os.path.join(
            self.project_root, "shots", "seq_1", "s1", "Anm", "work", "s1"
        )
        self.assertTrue(template.validate(test_path))
...
class TestApplyFields(TestTemplatePath):
    def test_simple(self):
        relative_path = os.path.join(
            "shots", "seq_1", "s1", "Anm", "work", "s1.mmm.v003.002.ma"
        )
    ...
    def test_working_enum(self):
        template = TemplatePath(
            "{Shot}/{Step}/file.ex", self.keys, root_path=self.project_root
        )
    ...
    def test_optional_values(self):
        ...
        del fields["snapshot"]
        relative_path = os.path.join(
            "shots", "seq_1", "s1", "Anm", "work", "s1.mmm.v003"
        )
        expected = os.path.join(self.project_root, relative_path)
        result = template_path.apply_fields(fields)
        self.assertEqual(expected, result)
        ...
        # remove optional value
        del fields["branch"]
        relative_path = os.path.join("shots", "seq_1", "s1", "Anm", "work", "s1.v003")
        expected = os.path.join(self.project_root, relative_path)
        result = template_path.apply_fields(fields)
        self.assertEqual(expected, result)
    ...
    def test_multi_key_optional(self):
        ...
        relative_path = os.path.join("shots", "s1.mmm.v003.002.ma")

        expected = os.path.join(self.project_root, relative_path)

        fields = {"Shot": "s1", "branch": "mmm", "version": 3, "snapshot": 2}
        result = template_path.apply_fields(fields)
        self.assertEqual(expected, result)

        # if one key from optional section missing, whole section skipped.
        relative_path = os.path.join("shots", "s1.002.ma")

        expected = os.path.join(self.project_root, relative_path)

        fields = {"Shot": "s1", "branch": "mmm", "snapshot": 2}
...
class Test_ApplyFields(TestTemplatePath):
    ...
    def test_ignore_type(self):
        relative_path = os.path.join(
            "shots", "seq_1", "shot_1", "Anm", "work", "shot_1.mmm.v*.002.ma"
        )
        expected = os.path.join(self.project_root, relative_path)
        fields = {
            "Sequence": "seq_1",
...
class TestGetFields(TestTemplatePath):
    def test_anim_path(self):
        relative_path = os.path.join(
            "shots", "seq_1", "shot_1", "Anm", "work", "shot_1.mmm.v003.002.ma"
        )
    ...
    def test_key_first(self):
        definition = (
            "{Sequence}/{Shot}/{Step}/work/{Shot}.{branch}.v{version}.{snapshot}.ma"
        )
        template = TemplatePath(definition, self.keys, root_path=self.project_root)
        relative_path = os.path.join(
            "seq_1", "shot_1", "Anm", "work", "shot_1.mmm.v003.002.ma"
        )
        file_path = os.path.join(self.project_root, relative_path)
        expected = {
            "Sequence": "seq_1",
    ...
    def test_missing_values(self):
        relative_path = os.path.join("shots", "seq_1", "Anm", "work", "mmm.v003.002.ma")
        file_path = os.path.join(self.project_root, relative_path)
        self.assertRaises(TankError, self.template_path.get_fields, file_path)

    def test_conflicting_values(self):
        relative_path = os.path.join(
            "shots", "seq_1", "shot_2", "Anm", "work", "shot_1.mmm.v003.002.ma"
        )
        file_path = os.path.join(self.project_root, relative_path)
        self.assertRaises(TankError, self.template_path.get_fields, file_path)
    ...
    def test_skip_key(self):
        relative_path = os.path.join(
            "shots", "seq_1", "shot_1", "Anm", "work", "shot_1.mmm.v003.###.ma"
        )
        file_path = os.path.join(self.project_root, relative_path)
        expected = {
            "Sequence": "seq_1",
    ...
    def test_different_case(self):
        relative_path = os.path.join(
            "Shots", "Seq_1", "Shot_1", "anim", "Work", "Shot_1.mmm.v003.002.ma"
        )
        file_path = os.path.join(self.project_root, relative_path)
        expected = {
            "Sequence": "Seq_1",
    ...
    def test_double_int_key(self):
        ...
        template = TemplatePath(
            definition, {"int_key": int_key}, root_path=self.project_root
        )
        expected = {"int_key": 4}
    ...
    def test_optional_sections(self):
        ...
        relative_path = os.path.join(
            "shots", "seq_1", "s1", "Anm", "work", "s1.mmm.002.ma"
        )
        input_path = os.path.join(self.project_root, relative_path)
        ...
        relative_path = os.path.join("shots", "seq_1", "s1", "Anm", "work", "s1")
        input_path = os.path.join(self.project_root, relative_path)
    ...
    def test_optional_with_underscore(self):
        ...
        """
        definition = (
            "sequences/{Sequence}/{Shot}/{Step}/work/{Shot}[_{name}].v{version}.nk"
        )
        template = tank.TemplatePath(definition, self.keys, self.project_root)
        # leave out optional 'name' field
        relative_path = os.path.join(
            "sequences", "seq_1", "shot_1", "Anm", "work", "shot_1.v001.nk"
        )
        input_path = os.path.join(self.project_root, relative_path)
        expected = {"Sequence": "seq_1", "Shot": "shot_1", "Step": "Anm", "version": 1}
        result = template.get_fields(input_path)
        self.assertEqual(expected, result)
...
class TestGetKeysSepInValue(TestTemplatePath):
    ...
    def test_double_key_first_precise(self):
        ...
        expected = {"Asset": "cat_man_fever", "name": "doogle"}
    ...
    def test_ambiguous(self):
        ...
        self.check_error_message(
            TankError, expected_msg, template.get_fields, input_path
        )
    ...
    def test_ambiguous_wrong_type(self):
        ...
        expected = {"Asset": "cat_1_man", "some_num": 14, "other_num": 2, "ext": "jpeg"}
        self.assert_path_matches(definition, input_path, expected)
    ...
    def test_ambigous_alphanum_first(self):
        ...
        expected = {"Asset": "cat_man_fever", "name_alpha": "doogle"}
        self.assert_path_matches(definition, input_path, expected)
...
class TestParent(TestTemplatePath):
    def test_parent_exists(self):
        expected_definition = os.path.join(
            "shots", "{Sequence}", "{Shot}", "{Step}", "work"
        )
