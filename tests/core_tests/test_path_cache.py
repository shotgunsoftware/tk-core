...
import sys
import time
from tank_vendor.third_party.six.moves.queue import Empty
...
from tank_vendor.third_party.six.moves import range
    ...
    def test_db_location(self):
        ...
        with mock.patch.object(
            ...
            self.assertEqual(
                expected,
                os.path.basename(
                    os.path.dirname(self.path_cache._get_path_cache_location())
                ),
            )
...
class TestAddMapping(TestPathCache):
    ...
    def test_dupe_failure(self):
        ...
        self.assertRaises(
            tank.TankError, add_item_to_cache, self.path_cache, ne2, full_path
        )
    ...
    def test_multi_entity_path(self):
        ...
        add_item_to_cache(
            self.path_cache, {"type": et, "id": eid, "name": en}, full_path
        )
        add_item_to_cache(
            self.path_cache,
            {"type": et, "id": eid + 1, "name": en},
            full_path,
            primary=False,
        )
        add_item_to_cache(
            self.path_cache,
            {"type": et, "id": eid + 2, "name": en},
            full_path,
            primary=False,
        )
        add_item_to_cache(
            self.path_cache,
            {"type": et, "id": eid + 3, "name": en},
            full_path,
            primary=False,
        )
        ...
        self.assertEqual(
            self.path_cache.get_entity(full_path),
            {"type": "EntityType", "id": 1, "name": "EntityName"},
        )

        # check lookup from other direction
        paths = self.path_cache.get_paths(
            self.entity["type"], self.entity["id"], primary_only=True
        )
        self.assertEqual(len(paths), 1)
        self.assertEqual(paths[0], full_path)

        paths = self.path_cache.get_paths(
            self.entity["type"], self.entity["id"] + 1, primary_only=True
        )
        self.assertEqual(len(paths), 0)

        paths = self.path_cache.get_paths(
            self.entity["type"], self.entity["id"] + 1, primary_only=False
        )
        self.assertEqual(len(paths), 1)
        self.assertEqual(paths[0], full_path)

        paths = self.path_cache.get_paths(
            self.entity["type"], self.entity["id"] + 2, primary_only=False
        )
        self.assertEqual(len(paths), 1)
        self.assertEqual(paths[0], full_path)

        paths = self.path_cache.get_paths(
            self.entity["type"], self.entity["id"] + 3, primary_only=False
        )
        self.assertEqual(len(paths), 1)
        self.assertEqual(paths[0], full_path)
        ...
        res = self.db_cursor.execute(
            "SELECT path, root FROM path_cache WHERE entity_type = ? AND entity_id = ?",
            (self.entity["type"], self.entity["id"] + 3),
        )
        self.assertEqual(len(res.fetchall()), 1)
    ...
    def test_add_utf_name(self):
        ...
        add_item_to_cache(
            self.path_cache,
            {"name": entity_name, "id": entity_id, "type": entity_type},
            full_path,
        )

        res = self.db_cursor.execute(
            "SELECT entity_name FROM path_cache WHERE entity_type = ? AND entity_id = ?",
...
class TestGetEntity(TestPathCache):
    ...
    def test_non_project_primary_path(self):
        ...
        non_proj = {
            "type": self.non_project["type"],
            "id": self.non_project["id"],
            "name": self.non_project["name"],
        }
        add_item_to_cache(self.path_cache, non_proj, shot_path)

        result = self.path_cache.get_entity(shot_path)
        self.assertIsNotNone(result)
        self.assertEqual(self.non_project["type"], result["type"])
        self.assertEqual(self.non_project["id"], result["id"])
        self.assertEqual(self.non_project["name"], result["name"])
    ...
    def test_add_and_find_project_primary_root(self):
        ...
        self.assertIsNotNone(result)
        self.assertEqual("Project", result["type"])
        self.assertEqual(self.project["id"], result["id"])
...
class TestGetPaths(TestPathCache):
    ...
    def test_add_and_find_project(self):
        ...
        result = self.path_cache.get_paths(
            "Project", self.project["id"], primary_only=True
        )
        self.assertIsNotNone(result)
...
class Test_SeperateRoots(TestPathCache):
    def test_different_case(self):
        ...
        self.assertEqual(os.sep + relative_path, relative_result)
...
class TestShotgunSync(TankTestBase):
    ...
    def test_shot(self):
        ...
        folder.process_filesystem_structure(
            self.tk, self.seq["type"], self.seq["id"], preview=False, engine=None
        )
    ...
    def test_no_new_folders_created(self):
        ...
        folder_events = self.tk.shotgun.find(
            "EventLogEntry", [["event_type", "is", "Toolkit_Folders_Create"]]
        )
    ...
    def test_missing_roots_mapping(self):
        ...
        self.pipeline_configuration._storage_roots = StorageRoots.from_metadata(
            invalid_roots_data
        )
        ...
        config_folder = os.path.join(self.pipeline_config_root, "config")

        # and set roots back again and check
        self.pipeline_configuration._storage_roots = StorageRoots.from_config(
            config_folder
        )
...
class TestPathCacheGetLocationsFullSync(TankTestBase):
    ...
    def test_get_entities(self):
        ...
        self.assertEqual(self._pc._get_filesystem_location_entities(folder_ids=[]), [])
        ...
        for entity in entities:
            ...
            entity_w_project = self.tk.shotgun.find_one(
                path_cache.SHOTGUN_ENTITY, [["id", "is", entity["id"]]], ["project"]
            )
            self.assertEqual(
                entity_w_project["project"]["id"],
                self.tk.pipeline_configuration.get_project_id(),
            )
...
class TestPathCacheDelete(TankTestBase):
    ...
    def test_simple_delete_by_paths(self):
        ...
        paths = self._pc.get_paths(
            self._shot_entity["type"], self._shot_entity["id"], primary_only=True
        )
        self.assertEqual(len(paths), 1)
    ...
    def test_unregister_multiple_folders(self):
        ...
        self.assertListEqual(
            self._pc.get_paths(
                shot2_entity["type"], shot2_entity["id"], primary_only=True
            ),
            [],
        )
...
class TestPathCacheBatchOperation(TankTestBase):
    ...
    def test_batched_shotgun_retrieval(self, find_mock):
        ...
        self.assertEqual(find_mock.call_count, 8)
        ...
        self.assertEqual(entities, ["dummy_data"] * 8)
