...
class TestStringKey(ShotgunTestBase):
    ...
    def test_choices_exclusions_conflict(self):
        ...
        exclusions = ["c", "a"]
        self.assertRaises(
            TankError, StringKey, "field_name", choices=choices, exclusions=exclusions
        )
    ...
    def test_str_from_value_bad_choice(self):
        ...
        self.check_error_message(
            TankError, expected, self.choice_field.str_from_value, value
        )
    ...
    def test_str_from_value_alphanum_bad(self):
        ...
        for bad_value in bad_values:
            ...
            self.check_error_message(
                TankError, expected, self.alphanum_field.str_from_value, bad_value
            )
    ...
    def test_str_from_value_alpha_bad(self):
        base_expected = (
            "%s Illegal value '%%s' does not fit filter_by 'alpha'" % self.alpha_field
        )
        bad_values = ["a2b", "a_b", "a b", "a-b", "*"]
        for bad_value in bad_values:
            expected = base_expected % bad_value
            self.check_error_message(
                TankError, expected, self.alpha_field.str_from_value, bad_value
            )
    ...
    def test_str_from_value_regex_bad(self):
        ...
        for bad_value in bad_values:
            expected = base_expected % bad_value
            self.check_error_message(
                TankError, expected, self.regex_field.str_from_value, bad_value
            )
    ...
    def test_shotgun_field_name_set(self):
        str_field = StringKey(
            "field_name", shotgun_entity_type="Shot", shotgun_field_name="code"
        )
        self.assertEqual("Shot", str_field.shotgun_entity_type)
    ...
    def test_subset(self):
        ...
        # test bad regex
        self.assertRaises(
            TankError, StringKey, "field_name", subset="({4}.).*BROKENREGEX"
        )
        ...
        tests.append(
            {
                ...
                "full": "John Smith",
                "template": StringKey(
                    "field_name", subset="([A-Z])[a-z]* ([A-Z])[a-z]*"
                ),
            }
        )
...
class TestIntegerKey(ShotgunTestBase):
    ...
    def test_str_from_value_bad(self):
        ...
        self.check_error_message(
            TankError, expected, self.int_field.str_from_value, value
        )
    ...
    def test_init_validation(self):
        ...
        self._validate_key(
            IntegerKey("version_number"), strict_matching=None, format_spec=None
        )
...
class TestSequenceKey(ShotgunTestBase):
    ...
    def test_str_from_value_bad(self):
        ...
        self.check_error_message(
            TankError, expected, self.seq_field.str_from_value, value
        )
...
class TestTimestampKey(ShotgunTestBase):
    ...
    def test_str_from_value(self):
        ...
        self.assertEqual(key.str_from_value(self._datetime), self._datetime_string)
