...
class TestSchemaCreateFoldersSingleStep(TankTestBase):
    ...
    def test_step_a(self):
        ...
        expected_paths.extend(
            [self.project_root, sequences_path, sequence_path, shot_path, step_path]
        )
    ...
    def test_step_b(self):
        ...
        folder.process_filesystem_structure(
            self.tk, self.task2["type"], self.task2["id"], preview=False, engine=None
        )
...
class TestSchemaCreateFoldersStepAndUserSandbox(TankTestBase):
    ...
    def test_step_a(self, get_current_user):
        ...
        folder.process_filesystem_structure(
            self.tk, self.task["type"], self.task["id"], preview=False, engine="foo-bar"
        )
