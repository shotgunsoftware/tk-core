...
class TestSchemaCreateFolders(TankTestBase):
    ...
    def test_unicode(self):
        ...
        expected_paths = self._construct_shot_paths(
            shot_name="\xe3\x81\xbe\xe3\x82\x93\xe3\x81\x88-foo-bar"
        )
    ...
    def test_asset(self):
        ...
        folder.process_filesystem_structure(
            self.tk, self.asset["type"], self.asset["id"], preview=False, engine=None
        )
    ...
    def test_scene(self):
        ...
        scene = {"type": "Scene", "id": 5, "code": "scenename", "project": self.project}
        ...
        expected_paths.append(
            os.path.join(self.project_root, "scenes", "scene_step_name")
        )
        expected_paths.append(
            os.path.join(self.project_root, "scenes", "scene_step_name", "scenename")
        )
        expected_paths.append(
            os.path.join(
                self.project_root, "scenes", "scene_step_name", "scenename", "work"
            )
        )

        folder.process_filesystem_structure(
            self.tk, scene["type"], scene["id"], preview=False, engine=None
        )
...
class TestCreateFilesystemStructure(TankTestBase):
    ...
    def test_create_task(self):
        ...
        expected = os.path.join(
            self.project_root, "sequences", self.seq["code"], self.shot["code"]
        )
    ...
    def test_create_asset(self):
        expected = os.path.join(
            self.project_root, "assets", self.asset["sg_asset_type"], self.asset["code"]
        )
        self.assertFalse(os.path.exists(expected))
        folder.process_filesystem_structure(
            self.tk, self.asset["type"], self.asset["id"], preview=False, engine=None
...
class TestSchemaCreateFoldersWorkspaces(TankTestBase):
    ...
    def test_shot(self):
        ...
        expected_paths = [self.project_root, os.path.join(self.project_root, "assets")]

        expected_paths.extend([sequences_path, sequence_path, shot_path, ws_path])
...
class TestFolderCreationPathCache(TankTestBase):
    ...
    def test_shotgun_path_cache_counts(self):
        ...
        res = self.db_cursor.execute("SELECT rowID, entity_type, path FROM path_cache ")
        ...
        filesystemlocation_entries = self.tk.shotgun.find(
            "FilesystemLocation", [], ["linked_entity_type"]
        )
        ...
        def check_match(fl_entity, shotgun_status_row, path_cache_row):
            ...
            """
            return (
                shotgun_status_row[1] == fl_entity["id"]
                and path_cache_row[1] == fl_entity["linked_entity_type"]
            )
        ...
        for path_cache_row in path_cache_entries:
            ...
            shotgun_status_rows = [
                s_row
                for s_row in shotgun_status_entries
                if path_cache_row[0] == s_row[0]
            ]
            ...
            filesystemlocation_entitys = [
                fl
                for fl in filesystemlocation_entries
                if check_match(fl, shotgun_status_rows[0], path_cache_row)
            ]
...
class TestFolderCreationEdgeCases(TankTestBase):
    ...
    def test_delete_shot_then_recreate(self):
        ...
        shot_path = os.path.join(
            self.project_root, "sequences", "seq_code", "shot_code"
        )
    ...
    def test_rename_shot_but_keep_on_disk(self):
        ...
        renamed_shot_path = os.path.join(
            self.project_root, "sequences", "seq_code", "shot_code_renamed"
        )
