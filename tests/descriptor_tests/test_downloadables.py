...
class TestDownloadableIODescriptors(ShotgunTestBase):
    ...
    # In Python 3.8 and 3.9, the TestCase class cannot be serialized due to a regression.
    # It appears an argument parser is reachable from the TestCase instance. The
    # multiprocessing module serializes both the test method and the instance associated to
    # to it (which makes sense, since we need the test data on the instance!). This becomes
    # an issue, because ArgumentParser is not picklable in Python 3.8 (due to a regression it seems).
    # The cleanest way to avoid this serialization issue without mucking around with the
    # TestCase internals and that generates the smallest diff is to
    # create a new Implementation class defined below that reimplements the necessary interface
    # from ShotgunTestBase and have the TestDownloadableIODescriptors call the test
...
class Implementation(object):
    """
    Class that actually contains the test and it's data.
    """
