...
class TestValidateSchema(ShotgunTestBase):
    ...
    def test_invalid_schema_type(self):
        ...
        self.check_error_message(
            TankError, expected_msg, validate_schema, self.app_name, schema
        )
    ...
    def test_invalid_default_value(self):
        ...
        value = {
            "type": "dict",
            "items": {"test_dict": {"type": "str", "default_value": 123}},
        }
        schema = {key: value}
    ...
    def test_list_invalid_schema(self):
        ...
        expected_msg = "Invalid type '%s' in schema '%s' for '%s'!" % (
            bad_type,
            key,
            self.app_name,
        )
        ...
        expected_msg = "Missing or invalid 'values' dict in schema '%s' for '%s'!" % (
            key,
            self.app_name,
        )
        self.check_error_message(
        ...
        expected_msg = "Invalid 'allows_empty' bool in schema '%s' for '%s'!" % (
            key,
            self.app_name,
        )
        self.check_error_message(
    ...
    def test_dict_invalid_schema(self):
        ...
        expected_msg = "Invalid 'items' dict in schema '%s' for '%s'!" % (
            key,
            self.app_name,
        )
        self.check_error_message(
        ...
        expected_msg = "Invalid 'bogus' dict in schema '%s' for '%s'" % (
            key,
            self.app_name,
        )
        self.check_error_message(
    ...
    def test_template_invalid_schema(self):
        ...
        expected_msg = "Invalid 'required_fields' list in schema '%s' for '%s'!" % (
            key,
            self.app_name,
        )
        self.check_error_message(
        ...
        expected_msg = (
            "Invalid 'required_fields' value '123' in schema '%s' for '%s'!"
            % (key, self.app_name)
        )
        self.check_error_message(
            TankError, expected_msg, validate_schema, self.app_name, schema
        ...
        expected_msg = "Invalid 'optional_fields' list in schema '%s' for '%s'!" % (
            key,
            self.app_name,
        )
        self.check_error_message(
        ...
        expected_msg = (
            "Invalid 'optional_fields' value '123' in schema '%s' for '%s'!"
            % (key, self.app_name)
        )
        self.check_error_message(
            TankError, expected_msg, validate_schema, self.app_name, schema
...
class TestValidateSettings(TankTestBase):
    ...
    def test_required_field_missing(self):
        ...
        expected_msg = (
            "Could not determine value for key '%s' in settings! No specified value and no default value."
            % key
        )
        self.check_error_message(
            TankError,
    ...
    def test_invalid_hook_syntax(self):
        ...
        hooks_dir = os.path.join(self.pipeline_config_root, "config", "hooks")
    ...
    def test_required_fields_not_in_template_keys(self):
        ...
        template = tank.template.TemplatePath(
            "{Shot}/work", self.keys, self.project_root
        )
        cfg_val = "template name"
    ...
    def test_required_field_missing_in_dict(self):
        ...
        schema = {key: {"type": "dict", "items": {missing_key: {"type": "str"}}}}
        settings = {key: {}}
    ...
    def test_skip_validate_context(self):
        ...
        # non-path template
        template = tank.template.TemplatePath(
            "name-{Shot}-somthing", self.keys, self.project_root
        )
        cfg_val = "template name"
        self.tk.templates = {cfg_val: template}
        # set up environment config
    ...
    def test_list_good_values(self):
        ...
        list_value = {
            "type": "list",
            "values": {"type": "list", "values": {"type": "int"}},
        }
        schema = {key: list_value}
    ...
    def test_list_bad_values(self):
        ...
        expected_msg = "The list in setting '%s' for '%s' can not be empty!" % (
            key,
            self.app_name,
        )
        self.check_error_message(
...
class TestValidateContext(TankTestBase):
    ...
    def test_required_fields(self):
        ...
        template = tank.template.TemplatePath(
            "{%s}" % field_name, self.keys, self.project_root
        )
        ...
        validate_settings(
            self.app_name, self.tk, self.context, self.metadata, self.config
        )
    ...
    def test_context_determines_fields(self):
        ...
        field_name = "Sequence"
        template = tank.template.TemplatePath(
            "sequence/{%s}" % field_name, self.keys, self.project_root
        )
        # tank instance with this template
        self.tk.templates = {self.template_name: template}
...
class TestValidateFixtures(TankTestBase):
    ...
    def test_environment(self):
        ...
        for app_name in env.get_apps(self.test_engine):
            schema = env.get_app_descriptor(
                self.test_engine, app_name
