...
class TestSetupProject(TankTestBase):
    ...
    @patch(
        ...
        new=mocked_resolve_core_path
    )
    def _test_setup_project(self, mocked=None):
        """
...
class TestSetupParams(TankTestBase):

    def test_project_name_hook(self):
        self.setup_fixtures(parameters={"core": "core.override/project_name_core"})

        params = tank.commands.setup_project_params.ProjectSetupParameters(
            logging.getLogger("/dev/null"), self.mockgun
        )

        params.set_associated_core_path(
            self.pipeline_config_root if sys.platform == "linux2" else None,
            self.pipeline_config_root if sys.platform == "win32" else None,
            self.pipeline_config_root if sys.platform == "darwin" else None,
        )
        params.set_project_id(self.project["id"], force=True)

        self.assertEqual(params.get_default_project_disk_name(), "project_name_from_hook")
