...
def __current_version_less_than(log, sgtk_install_root, ver):
    ...
    if __is_upgrade(sgtk_install_root) == False:
        ...
        log.debug(
            "There is no current version. This is the first time the core is being installed."
        )
        return True
...
def _upgrade_path_cache(log):
    ...
    log.info("")
    log.info(
        "Once Toolkit 0.15 has been installed, all new projects will automatically "
    )
    log.info(
        "have this feature enabled. Existing projects can optionally use this feature "
    )
...
def __copy_tank_cmd_binaries(src_dir, dst_dir, tank_scripts, log):
    ...
    for tank_script in tank_scripts:
        ...
        if not os.path.exists(dst_tank_script):
            log.warning(
                "   Could not find file: '%s' to replace, skipping!" % dst_tank_script
            )
            continue
...
def upgrade_tank(sgtk_install_root, log):
    ...
    try:
        ...
        if __is_upgrade(sgtk_install_root) and __current_version_less_than(
            log, sgtk_install_root, "v0.14.72"
        ):
            log.error(
                "You are running a very old version of the Toolkit Core API. Automatic upgrades "
                "are no longer supported. Please contact support at https://support.shotgridsoftware.com"
            )
            return

        if __is_upgrade(sgtk_install_root) and __current_version_less_than(
            log, sgtk_install_root, "v0.15.0"
        ):
        ...
        if not os.path.exists(os.path.join(sgtk_install_root)):
            log.error(
                "The specified sgtk install root '%s' doesn't look valid!\n"
        ...
        if os.path.exists(core_install_location):
            ...
            log.info(
                "Backing up Core API: %s -> %s" % (core_install_location, backup_path)
            )
