...
def get_content_templates_location(project_path):
    """
    returns the location of the content templates file
    """
    return os.path.join(project_path, "tank", "config", "core", CONTENT_TEMPLATES_FILE)

def get_core_hooks_folder(project_path):
    """
    returns the core hooks folder for the project
    """
    return os.path.join(project_path, "tank", "config", "core", "hooks")
