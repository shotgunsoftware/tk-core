...
# Ensure that we get the correct linux key, even if sys.platform returns just "linux2"
platform = "linux2" if sys.platform.startswith("linux2") else sys.platform
...
if not os.path.exists(parent_cfg_path):
    raise Exception(
        "Sgtk: Cannot find referenced core configuration file '%s'!" % parent_cfg_path
    )
...
if not os.path.exists(parent_python_path):
    raise Exception(
        "Sgtk: Cannot find referenced core location '%s'" % parent_python_path
    )
