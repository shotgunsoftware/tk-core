...
parent_file_name =  "parent_%s.cfg" % sys.platform
parent_cfg_path = os.path.join(current_folder, "..", "..", "..", "..", "config", "system", parent_file_name)
parent_cfg_path = os.path.abspath(parent_cfg_path)

if not os.path.exists(parent_cfg_path):
    raise Exception("Tank: Cannot find parent configuration file '%s'!" % parent_cfg_path)
...
if not os.path.exists(parent_python_path):
    raise Exception("Tank: Cannot find parent location '%s'" % parent_python_path)
