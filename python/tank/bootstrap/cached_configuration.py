    ...
    def cleanup_backup_folders(self):
        ...
        if self.config_backup_folder_path is not None:
            try:
                ...
                log.error("Failed to clean up the previous installation folder at: %s" % self.config_backup_folder_path)
        if self.core_backup_folder_path is not None:
            try:
                filesystem.delete_folder(self.core_backup_folder_path)
                self.core_backup_folder_path = None
            except Exception, e:
                log.error("Failed to clean up the previous installation folder at: %s" % self.core_backup_folder_path)
