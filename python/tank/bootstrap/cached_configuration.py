...
class CachedConfiguration(Configuration):
    ...
    def update_configuration(self):
        ...
        try:
            ...
            self._cleanup_backup_folders(os.path.dirname(config_backup_path) if config_backup_path else None, core_backup_path)
    ...
    def _cleanup_backup_folders(self, config_backup_folder_path, core_backup_folder_path):
        ...
        :param core_backup_folder_path:   Path to the core backup folder to be deleted.
        """
        if config_backup_folder_path is not None:
            try:
                filesystem.delete_folder(config_backup_folder_path)
            except Exception, e:
                log.error("Failed to clean up the previous installation folder at: %s. %s" % (config_backup_folder_path, e))
        if core_backup_folder_path is not None:
            try:
                filesystem.delete_folder(core_backup_folder_path)
            except Exception, e:
                log.error("Failed to clean up the previous installation folder at: %s. %s" % (core_backup_folder_path, e))
