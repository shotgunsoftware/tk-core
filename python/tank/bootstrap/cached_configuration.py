...
class CachedConfiguration(Configuration):
    ...
    def update_configuration(self):
        ...
        try:
            ...
            # in a usable state.
            core_backup_path = self._config_writer.move_to_backup(undo_on_error=True)
        except Exception as e:
            log.exception(
        ...
        try:
            ...
            self._cleanup_backup_folders(core_backup_path)
    ...
    def _cleanup_backup_folders(self, core_backup_folder_path):
        """
        ...
        """
        if core_backup_folder_path:
            try:
                filesystem.safe_delete_folder(core_backup_folder_path)
                log.debug("Deleted backup folder: %s", core_backup_folder_path)
            except Exception as e:
                log.warning("Failed to clean up temporary backup folder '%s': %s" % (core_backup_folder_path, e))
