...
class ConfigurationResolver(object):
    ...
    def resolve_configuration(self, config_descriptor, sg_connection):
        ...
        if config_descriptor["type"] == constants.BAKED_DESCRIPTOR_TYPE:
            ...
            if is_descriptor_version_missing(config_descriptor):
                ...
                # Check if we should block auto-update based on Python version compatibility
                compatible_version = self._get_python_compatible_config_version(
                    sg_connection, config_descriptor
                )
                log.info(
                    f"COMPATIBLE VERSION {compatible_version} for descriptor {config_descriptor}")
                if compatible_version:
                    log.info(
                        "Auto-update blocked: Current Python %s is not compatible with latest config. "
                        "Using compatible version %s instead." 
                        % (".".join(str(i) for i in sys.version_info[:2]), compatible_version)
                    )
                    config_descriptor["version"] = compatible_version
                    resolve_latest = False
            else:
                log.debug(
    ...
    def _get_python_compatible_config_version(self, sg_connection, config_descriptor):
        """
        Checks if the latest version of the config is compatible with the current Python version.
        If not, attempts to find the most recent compatible version.
        
        This implements SG-32871: reads minimum_python_version from config info.yml files
        to determine compatibility instead of hardcoding version numbers.
        
        :param sg_connection: Shotgun API instance
        :param config_descriptor: Configuration descriptor dict  
        :return: Compatible version string if latest is incompatible, None if compatible or can't determine
        """
        current_python = sys.version_info[:2]
        
        try:
            # First, check if the latest version is compatible
            temp_descriptor = create_descriptor(
                sg_connection,
                Descriptor.CONFIG,
                config_descriptor,
                fallback_roots=self._bundle_cache_fallback_paths,
            ...
            # Download the latest version to check its info.yml
            if not temp_descriptor.exists_local():
                temp_descriptor.download_local()
            
            # Check if latest version has minimum_python_version requirement
            info_yml_path = os.path.join(temp_descriptor.get_path(), "info.yml")
            if os.path.exists(info_yml_path):
                ...
                if min_python_version:
                    # Parse the minimum version (e.g., "3.8" or "3.8.0")
                    ...
                    if current_python < required_version:
                        log.debug(
                            "Latest config version %s requires Python %s, but running %s" 
                            % (temp_descriptor.version, min_python_version, 
                               ".".join(str(i) for i in current_python))
                        )
                        
                        # Try to find a compatible version by checking previous versions
                        return self._find_compatible_config_version(
                            sg_connection, config_descriptor, current_python
                        )
                    else:
                        log.debug(
                            "Latest config version %s is compatible with Python %s" 
                            % (temp_descriptor.version, ".".join(str(i) for i in current_python))
                        )
                        return None
                else:
                    # No minimum_python_version specified, assume compatible
                    log.debug("Config has no minimum_python_version, assuming compatible")
                    return None
            else:
                log.debug("Could not find info.yml, assuming compatible")
                return None
                
        except Exception as e:
            log.debug("Error checking Python compatibility: %s, allowing update" % e)
            return None

    def _find_compatible_config_version(self, sg_connection, config_descriptor, current_python):
        """
        Attempts to find the most recent config version that is compatible with current Python.
        
        This method iterates through available versions in descending order and checks each
        version's info.yml for minimum_python_version compatibility.
        
        :param sg_connection: Shotgun API instance
        :param config_descriptor: Configuration descriptor dict
        :param current_python: Current Python version tuple (major, minor)
        :return: Compatible version string or None if not found
        """
        try:
            # Create a descriptor to get available versions
            temp_descriptor = create_descriptor(
                sg_connection,
                Descriptor.CONFIG,
                config_descriptor,
            ...
            # Get all available versions, sorted in descending order (newest first)
            available_versions = temp_descriptor.find_latest_cached_version(
                allow_prerelease=False
            )
            
            if not available_versions:
                log.debug("No cached versions found, trying to get version list from source")
                # If no cached versions, we need to get the version list from the source
                # This depends on the descriptor type (git, app_store, etc.)
                try:
                    # For git-based configs, we can list tags/branches
                    available_versions = temp_descriptor.get_version_list()
                except Exception as e:
                    log.debug("Could not get version list: %s" % e)
                    return None
            
            if isinstance(available_versions, str):
                available_versions = [available_versions]
            elif not available_versions:
                log.debug("No versions available to check")
                return None
            
            # Sort versions in descending order (most recent first)
            # This is a simple string sort which works for most version schemes
            available_versions = sorted(available_versions, reverse=True)
            
            log.debug("Checking %d versions for Python %s compatibility" % 
                     (len(available_versions), ".".join(str(i) for i in current_python)))
            
            # Check each version starting from the newest
            for version in available_versions:
                try:
                    # Create descriptor for this specific version
                    version_descriptor_dict = config_descriptor.copy()
                    version_descriptor_dict["version"] = version
                    
                    version_descriptor = create_descriptor(
                        sg_connection,
                        Descriptor.CONFIG,
                        version_descriptor_dict,
                        fallback_roots=self._bundle_cache_fallback_paths,
                        resolve_latest=False,
                    )
                    
                    # Download if not already local
                    if not version_descriptor.exists_local():
                        log.debug("Downloading version %s to check compatibility" % version)
                        version_descriptor.download_local()
                    
                    # Check this version's minimum_python_version
                    info_yml_path = os.path.join(version_descriptor.get_path(), "info.yml")
                    if os.path.exists(info_yml_path):
                        from .. import yaml_cache
                        version_info = yaml_cache.g_yaml_cache.get(info_yml_path, deepcopy_data=False)
                        
                        min_python_version = version_info.get('minimum_python_version')
                        if min_python_version:
                            ...
                            if current_python >= required_version:
                                log.debug(
                                    "Found compatible version %s (requires Python %s, running %s)" 
                                ...
                                log.debug(
                                    "Version %s requires Python %s, skipping" 
                                    % (version, min_python_version)
                                )
                        else:
                            # No minimum_python_version specified, assume this version is compatible
                            log.debug(
                                "Version %s has no minimum_python_version, assuming compatible" 
                                % version
                            )
                            return version
                    else:
                        # No info.yml found, assume compatible (older versions might not have it)
                        log.debug(
                            "Version %s has no info.yml, assuming compatible" 
                            % version
                        )
                        return version
                        
                except Exception as e:
                    log.debug("Error checking version %s: %s" % (version, e))
                    continue
            
            # No compatible version found
            log.debug("No compatible version found for Python %s" % 
                     ".".join(str(i) for i in current_python))
            return None
            
        except Exception as e:
            log.debug("Error finding compatible version: %s" % e)
            return None
