...
def download_url(sg, url, location, use_url_extension=False, headers={}):
    """
...
def download_and_unpack_url(sg, url, target, retries=5, auto_detect_bundle=False, headers={}):
    """
    ...
    :raises: ShotgunAttachmentDownloadError on failure
    """
    return _download_and_unpack(sg, target, retries, auto_detect_bundle, url=url, headers=headers)


@LogManager.log_timing
def _download_and_unpack(
    sg, target, retries, auto_detect_bundle, attachment_id=None, url=None, headers={}
):
    """
    ...
    while not invalid_zip_file and not done and attempt < retries:
        ...
        try:
            ...
            if attachment_id:
                ...
                download_url(sg, url, zip_tmp, headers=headers)
            else:
                raise ValueError(
